...
assert.ok(
  appSource.includes("report.economics.lines.map") || appSource.includes("economics.enterpriseValueBand"),
  "HTML Deal Economics must render legacy report.economics.lines or Mergevue adapter economics fields.",
);
assert.ok(
...
assert.ok(
  appSource.includes("economics: buildDealEconomicsReport(session, { baseEcsScore: score })")
    || appSource.includes("const report = buildMergevuePublicReportModel(session, { deliverable })"),
  "Public report must use legacy Deal Economics score binding or the Mergevue adapter model.",
);
