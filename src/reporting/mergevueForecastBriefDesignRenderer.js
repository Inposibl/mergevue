...
function renderHtmlSection(section, number, context = {}) {
  ...
  if (section.id === "environments") {
    ...
    return `<section class="sec" id="environments" data-screen-label="Identified Environment Types">${sectionHead(number, section.title, (cleanText(section.acquirer && section.acquirer.environment) === cleanText(section.target && section.target.environment) && cleanText(section.acquirer && section.acquirer.environment)) ? "Convergent operating model" : ARCHIVE_SECTION_NOTES.environments)}<div class="envs">${renderEnvironmentCard("Acquirer", section.acquirer)}${renderEnvironmentCard("Target", section.target)}</div><div class="panel env-bridge"><strong>Core mismatch.</strong> ${escapeHtml(section.coreMismatch || "The core mismatch depends on the two identified operating environments.")}</div></section>`;
  }
