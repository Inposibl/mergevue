...
function renderResourceConflictSummary(section) {
  ...
  const ecsClarifier = "ECS is a compatibility score: higher means stronger alignment. Resource scores are contestation-intensity scores: higher means greater integration pressure. The two scales are related but intentionally read in opposite directions. ECS is not an average of the displayed resources. It reflects compatibility across the full 17-resource model; this page displays selected resources where integration pressure is most likely to damage or preserve value. Public preview displays selected exposed resources. The resource impact vectors behind each resource are currently expert-coded from the structural environment model; the paid workflow reviews them against operating artifacts and decomposes the per-resource drivers behind the score."; return `<div class="resource-summary"><h4>What this means in practice</h4><p>${escapeHtml(intro)} ${escapeHtml(ecsClarifier)}</p><div class="resource-summary-grid">${rows}</div></div>`;
}
