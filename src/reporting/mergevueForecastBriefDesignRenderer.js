...
function renderHtmlSection(section, number, context = {}) {
  ...
  if (section.id === "timeline") {
    ...
    return `<section class="sec" id="timeline" data-screen-label="Timeline of Proposed Actions">${sectionHead(number, "Timeline of Proposed Actions", ARCHIVE_SECTION_NOTES.timeline)}<div class="legend"><span>${escapeHtml(section.timingLogic.signalSetup)}</span><span>${escapeHtml(section.timingLogic.observationWindow)}</span><span>${escapeHtml(section.timingLogic.verificationDeadline)}</span></div><div class="tl">${columns}</div><div class="timeline-actions"><h4 class="timeline-actions-title">Proposed actions</h4><div class="acts">${renderActionPanel("Before close", actions.beforeClose ?? [])}${renderActionPanel("After close", actions.afterClose ?? [])}</div><div class="cta"><div><div class="cl">Recommended next action</div><div class="ct">${escapeHtml(actions.beforeClose?.[0]?.actionTitle || actions.afterClose?.[0]?.actionTitle)}</div></div><div class="cbtn">Book practitioner session</div></div></div></section>`;
  }
