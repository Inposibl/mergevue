...
function scoreDisplayValue(value) {
  return value === null || value === undefined ? "Not available" : value;
}

function MergevueActionList({ actions }) {
  return (
    <div className="client-output-stack">
      {actions.map((action) => (
        <article key={`${action.actionTitle}-${action.actionTiming}`}>
          <strong>{action.actionTitle}</strong>
          <p><strong>Timing:</strong> {action.actionTiming}</p>
          <p><strong>Owner:</strong> {action.actionOwner}</p>
          <p><strong>Reason:</strong> {action.actionReason}</p>
          <p><strong>Expected effect:</strong> {action.actionExpectedEffect}</p>
        </article>
      ))}
    </div>
  );
  ...
  return (
    <div className="forecast-report-document" style={FORECAST_REPORT_STYLES.document}>
      <section className="reveal-block forecast-cover-block" style={FORECAST_REPORT_STYLES.cover}>
        ...
        <h2 style={FORECAST_REPORT_STYLES.coverTitle}>{brand.name}</h2>
        <div style={FORECAST_REPORT_STYLES.coverMeta}>
          <p style={FORECAST_REPORT_STYLES.coverLead}><strong>{brand.product}</strong> / {brand.reportType}</p>
          <p style={FORECAST_REPORT_STYLES.coverLead}>{scenario.acquirerName} acquiring {scenario.targetName}</p>
          <p style={FORECAST_REPORT_STYLES.coverLead}><strong>Forecast signal:</strong> {executive.headline}</p>
          <p style={FORECAST_REPORT_STYLES.coverLead}><strong>Decision implication:</strong> {executive.decisionImplication}</p>
          <p style={FORECAST_REPORT_STYLES.scoreChip}><span>Compatibility score</span><span>{scoreDisplayValue(scenario.compatibilityScore)} / {scenario.compatibilityBand}</span></p>
        </div>
        <p className="source-note" style={FORECAST_REPORT_STYLES.muted}>{brand.contactEmail} / {footer.generatedAt}</p>
      </section>

      <ForecastReportSection number={nextSectionNumber()} title={MERGEVUE_PUBLIC_REPORT_BLOCKS[0]}>
        <p><strong>{executive.headline}</strong></p>
        <p>{executive.oneParagraphSummary}</p>
        <p><strong>Decision implication:</strong> {executive.decisionImplication}</p>
        <p><strong>Main risk:</strong> {executive.mainRisk}</p>
        <p><strong>Recommended action:</strong> {executive.recommendedAction}</p>
      </ForecastReportSection>

      <ForecastReportSection number={nextSectionNumber()} title={MERGEVUE_PUBLIC_REPORT_BLOCKS[1]} variant="dashed">
        <ForecastFieldRows rows={[
          ["Status", sealed.statusTitle],
          ["Description", sealed.statusDescription],
        ]} />
        <div className="client-output-stack">
          {sealed.predictions.map((prediction) => (
            <article key={`${prediction.predictionTitle}-${prediction.predictionWindow}`}>
              <strong>{prediction.predictionTitle}</strong>
              <p><strong>Window:</strong> {prediction.predictionWindow}</p>
              <p><strong>Claim:</strong> {prediction.predictionClaim}</p>
              <p><strong>Observable signal:</strong> {prediction.observableSignal}</p>
              <p><strong>Verification method:</strong> {prediction.verificationMethod}</p>
            </article>
          ))}
        </div>
      </ForecastReportSection>

      <ForecastReportSection number={nextSectionNumber()} title={MERGEVUE_PUBLIC_REPORT_BLOCKS[2]}>
        <ForecastFieldRows rows={[
          ["Acquirer", scenario.acquirerName],
          ["Target", scenario.targetName],
          ["Deal type", scenario.dealType],
          ["Enterprise value", scenario.enterpriseValueBand],
          ["Data quality", scenario.dataQuality],
      ...
      <ForecastReportSection number={nextSectionNumber()} title={MERGEVUE_PUBLIC_REPORT_BLOCKS[3]}>
        <div className="protocol-insight-grid" style={FORECAST_REPORT_STYLES.twoColumn}>
          <article style={FORECAST_REPORT_STYLES.soberCard}>
            <strong>Acquirer environment: {environments.acquirerEnvironmentName}</strong>
            <p><strong>Code:</strong> {environments.acquirerEnvironmentCode}</p>
            <p>{environments.acquirerEnvironmentDescription}</p>
            <p><strong>Behavior pattern:</strong> {environments.acquirerBehaviorPattern}</p>
          </article>
          <article style={FORECAST_REPORT_STYLES.soberCard}>
            <strong>Target environment: {environments.targetEnvironmentName}</strong>
            <p><strong>Code:</strong> {environments.targetEnvironmentCode}</p>
            <p>{environments.targetEnvironmentDescription}</p>
            <p><strong>Behavior pattern:</strong> {environments.targetBehaviorPattern}</p>
          </article>
        </div>
      </ForecastReportSection>

      <ForecastReportSection number={nextSectionNumber()} title={MERGEVUE_PUBLIC_REPORT_BLOCKS[4]}>
        <p><strong>{collision.collisionHeadline}</strong></p>
        <p>{collision.collisionSummary}</p>
        <p><strong>Primary tension:</strong> {collision.primaryTension}</p>
        <p><strong>Why it matters:</strong> {collision.whyItMatters}</p>
      ...
      <ForecastReportSection number={nextSectionNumber()} title={MERGEVUE_PUBLIC_REPORT_BLOCKS[6]}>
        <ForecastFieldRows rows={[
          ["Signal setup", timeline.timingLogic.signalSetup],
          ["Observation window", timeline.timingLogic.observationWindow],
          ["Verification deadline", timeline.timingLogic.verificationDeadline],
        ]} />
        <div className="anchor-list compact-anchors">
          {timeline.phases.map((phase) => (
            <article key={`${phase.phaseName}-${phase.timeWindow}`}>
              <strong>{phase.phaseName} / {phase.timeWindow}</strong>
              <p><strong>Expected friction:</strong> {phase.expectedFriction}</p>
              <p><strong>Observable signal:</strong> {phase.observableSignal}</p>
              <p><strong>Recommended check:</strong> {phase.recommendedCheck}</p>
            </article>
          ))}
        </div>
      </ForecastReportSection>
      ...
      <ForecastReportSection number={nextSectionNumber()} title={MERGEVUE_PUBLIC_REPORT_BLOCKS[8]}>
        <MergevueActionList actions={report.recommendedActions} />
      </ForecastReportSection>
      ...
      <ForecastReportSection number={nextSectionNumber()} title={MERGEVUE_PUBLIC_REPORT_BLOCKS[9]}>
        ...
        <p><strong>What this report cannot say:</strong> {evidence.whatThisReportCannotSay}</p>
      </ForecastReportSection>

      <ForecastReportSection number={nextSectionNumber()} title={MERGEVUE_PUBLIC_REPORT_BLOCKS[10]} variant="premium">
        <ForecastBulletList items={engagement.benefits} />
        <p>{engagement.cta}</p>
        <p>{engagement.contactEmail}</p>
      </ForecastReportSection>

      <ForecastReportSection number={nextSectionNumber()} title={MERGEVUE_PUBLIC_REPORT_BLOCKS[11]}>
        <ForecastFieldRows rows={[
          ["Report ID", footer.reportId],
          ["Generated at", footer.generatedAt],
          ["Report version", footer.reportVersion],
          ["Scenario ID", footer.scenarioId],
          ["Brand", footer.brand],
          ["Contact", footer.contactEmail],
          ["Public URL pattern", footer.publicUrlPattern],
          ["Track record URL", footer.trackRecordUrl],
        ]} />
      </ForecastReportSection>
    </div>
  );
...
function createSimplePdf(lineItems) {
  ...
  function addFooter() {
    pages.forEach((page, index) => {
      const footerLeft = `mergevue.com  \u00b7  report@mergevue.com  \u00b7  Page ${index + 1}`;
      const fontSize = 8;
      const ruleY = 54;
      const textY = 38;
...
function buildFinalDeliverablesReportLines(deliverable, session) {
  if (!deliverable?.ready) {
    return [
      { text: "MERGEVUE", size: 9, bold: true, align: "center", color: PDF_BRAND.accent, after: 20, maxCharacters: 90 },
      { text: "FORECAST BRIEF", size: 26, bold: true, align: "center", color: PDF_BRAND.navy, after: 12, maxCharacters: 44 },
...
function createForecastBriefVisualHtml(deliverable, session) {
  ...
  if (typeof html !== "string" || !html.trim()) {
    throw new Error("Forecast Brief HTML renderer returned empty output.");
  }
...
async function createForecastBriefVisualPdfBlob(deliverable, session) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`PDF render service failed (${response.status}). ${detail}`.trim());
  }

  const contentType = response.headers.get("content-type") || "";
  if (!contentType.toLowerCase().includes("application/pdf")) {
    throw new Error(`PDF render service returned unexpected content type: ${contentType || "unknown"}.`);
  }

  const blob = await response.blob();
  if (blob.size < 10_000) {
    throw new Error("PDF render service returned an unexpectedly small PDF file.");
  }
